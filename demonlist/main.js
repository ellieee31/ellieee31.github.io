...
async function getAredlData(levelID) {
    try {
        const response = await fetch(
            "https://api.aredl.net/v2/api/aredl/levels/" + levelID,
        );
        if (!response.ok) {
            ...
            for (i = 0; i < spreadsheetResults.length; i++) {
                ...
                if (url.includes("demonlist")) {
                    mainlevels.push({
                        ...
                        tungname: spreadsheetData.tungName,
                        youtube: spreadsheetData.youtube,
                        changelog: spreadsheetData.changelog,
                    });
                    levelsNames.push(aredlData.name);
                    time = mainlevels[i].playtimeSorting * 86400;
                    mainlevels[i].playtime = playtimeCalc(
                        mainlevels[i].playtime,
                    );
                    sheetsDate = new Date(Date.UTC(1899, 11, 30, 0, 0, 0, 0));
                    mainlevels[i].date = new Date(
                        (mainlevels[i].date - 25569) * 86400000,
                    );
...
async function buildMain(timeMachine) {
    ...
    if (mainlevels.length == 0)
        await collectData(
            `https://opensheet.elk.sh/1QbdEQf6EbJJJ25ko4LnEoiStcZFyheAZoFozWy_59n4/demonlist?raw=true`,
        );
    ...
    mainlevels.forEach((level) => {
        ...
        if (tokimode) {
            ...
            html += `<h3 class="levelname">${level.tungname} by ${level.creator} ${level.hardest}</h3>`;
        } else {
        ...
        html += `
            ...
            <div>Attempts: ${level.attempts}`;
        if (level.attempts == Math.min(...attemptsAll)) {
            html += ` ⬇️`;
        } else if (level.attempts == Math.max(...attemptsAll)) {
            html += ` ⬆️`;
        }
        html += `</div><div>Enjoyment: ${level.enjoyment}/100`;
        if (level.enjoyment == Math.min(...enjoymentAll)) {
            html += ` ⬇️`;
        } else if (level.enjoyment == Math.max(...enjoymentAll)) {
            html += ` ⬆️`;
        }
        html += `</div><div>Max BPM: ${level.bpm} BPM`;
        if (level.bpm == Math.min(...bpmAll)) {
            html += ` ⬇️`;
        } else if (level.bpm == Math.max(...bpmAll)) {
            html += ` ⬆️`;
        }
        html += `</div><div>Worst Death: ${level.worstDeath}%`;
        if (level.worstDeath == Math.min(...failAll)) {
            html += ` ⬇️`;
        } else if (level.worstDeath == Math.max(...failAll)) {
            html += ` ⬆️`;
        }
        html += `</div><div>Hardest Part: ${level.hardestPart}%</div>`;
        html += `</div>
                        <div class="timestats">
                            <div class="playtime">Level Playtime: ${level.playtime}`;
        if (level.playtimeSorting == Math.min(...playtimeAll)) {
            html += ` ⬇️`;
        } else if (level.playtimeSorting == Math.max(...playtimeAll)) {
            html += ` ⬆️`;
        }
    ...
    document
        .querySelectorAll(
            "#mainlist .level, #top5progression .level, #inprogress .level",
        )
        .forEach((level) => {
            level.addEventListener("click", function (e) {
                ...
                document.getElementById("listexpand").textContent =
                    "expand all";
                allExpanded = false;
            });
        });
...
function getLevel(value, argument) {
    if (
        mainlevels.filter((element) => element[argument] == value).length == 1
    ) {
        return mainlevels.filter((element) => element[argument] == value)[0]
            .name;
    } else {
        return (
            mainlevels.filter((element) => element[argument] == value).length +
            " Levels"
        );
    }
}
...
async function buildOther() {
    if (inprogress.length == 0)
        await collectData(
            `https://opensheet.elk.sh/1QbdEQf6EbJJJ25ko4LnEoiStcZFyheAZoFozWy_59n4/progress?raw=true`,
        );
    if (upcoming.length == 0)
        await collectData(
            `https://opensheet.elk.sh/1QbdEQf6EbJJJ25ko4LnEoiStcZFyheAZoFozWy_59n4/upcoming?raw=true`,
        );
    if (bucketlist.length == 0)
        await collectData(
            `https://opensheet.elk.sh/1QbdEQf6EbJJJ25ko4LnEoiStcZFyheAZoFozWy_59n4/bucketlist?raw=true`,
        );
    allLevels = allLevels.concat(mainlevels, inprogress, upcoming, bucketlist);
    if (packs.length == 0)
        await collectData(
            `https://opensheet.elk.sh/1QbdEQf6EbJJJ25ko4LnEoiStcZFyheAZoFozWy_59n4/packs?raw=true`,
        );
...
document.getElementById("tung").addEventListener("click", function () {
    tungmode = !tungmode;
    tokimode = false;
    buildMain();
    buildPacks();
...
document.getElementById("top5").addEventListener("click", function () {
    if (
        ...
        document.getElementById("top5progression").style.display == ""
    ) {
...
document.getElementById("packsButton").addEventListener("click", function () {
    if (
        document.getElementById("packs").style.display == "none" ||
        document.getElementById("packs").style.display == ""
    ) {
        document.getElementById("mainlist").style.display = "none";
        document.getElementById("top5progression").style.display = "none";
...
document.getElementById("timetravel").addEventListener("click", function () {
    buildMain(
        new Date(document.getElementById("timemachinedate").value.toString()),
    );
});
...
function expandAll() {
    if (document.getElementById("mainlist").style.display != "none") {
        ...
        if (allExpanded == true) {
            document
                .querySelectorAll("#top5progression .level")
                .forEach((level) => {
                    level.classList.add("active");
                });
            document.getElementById("listexpand").textContent = "collapse all";
...
function buildTop5() {
    ...
    dates.forEach((date) => {
        ...
        if (
            JSON.stringify(previousList) != JSON.stringify(currentList) &&
